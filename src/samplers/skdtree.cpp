...
CVector CKdTree::GenerateNewSamplePosition(MTRand& mtrand, bool Is2D)
{
  CVector Result;
  float MaxDistance;
  ...
  //for (int m = 0; m < 3; ++m)
  {
    ...
    CVector ParentPosition = 0.5f * (this->Nodes[ParentNodeIndex].BBox.Max + this->Nodes[ParentNodeIndex].BBox.Min);
...
void CKdTree::SortSamples(std::vector<int>& SamplesIndices, int Axis, int li, int ri)
{
	if ((li - ri) < 50)
	{
		// insertion sort
...
void CKdTree::Build(const std::vector<CSample> SamplesList)
{
  //this->Nodes.clear();
  //this->Samples.clear();
...
int CKdTree::MakeNodeRecon(std::vector<int> SamplesIndices, const int Axis, const CBBox BBox)
{
...
int CKdTree::MakeNode(std::vector<int> SamplesIndices, const int Axis, const CBBox BBox)
{
  int CurrentIndex = this->CurrentNumNodes;
  this->CurrentNumNodes++;
...
int CKdTree::MakeNode2D(std::vector<int> SamplesIndices, const int Axis, const CBBox BBox)
{
...
void CKdTree::Insert(CSample& Sample, bool Is2D)
{
	// add a new sample
...
void CKdTree::SearchTest(const int SampleIndex, const CVector & QueryPosition, CKNNs* KNNs)
{
...
void CKdTree::SearchTestAAEllipse(const int SampleIndex, const CVector & QueryPosition, CKNNs* KNNs, const CVector & scale)
{
  // compute scaled difference vector
  CVector diff = this->Samples[SampleIndex].Position - QueryPosition;
  for (int i = 0; i < DIMENSION; ++i)
...
void CKdTree::SearchTestEllipse(const int SampleIndex, const CVector & QueryPosition, CKNNs* KNNs, const CMatrix & M, const CMatrix & MInv)
{
...
void CKdTree::SearchIterationAAEllipse(const int NodeIndex, const CVector & QueryPosition, CKNNs* KNNs, const int RequiredKNNsNum, const CVector & scale)
{
  if (this->Nodes[NodeIndex].NodeType == NodeTypeLeaf)
  {
    int LeafNodeSamplesNum = (int)this->Nodes[NodeIndex].SamplesIndices.size();
    for (int i = 0; i < LeafNodeSamplesNum; ++i)
    {
      ...
      else
      {
        ...
        // compute scaled difference vector
        CVector diff = this->Samples[NewSampleIndex].Position - QueryPosition;
        for (int i = 0; i < DIMENSION; ++i)
          diff.Elements[i] *= scale.Elements[i];
...
void CKdTree::SearchIterationEllipse(const int NodeIndex, const CVector & QueryPosition, CKNNs* KNNs, const int RequiredKNNsNum, const CMatrix & M, const CMatrix & MInv)
{
  if (this->Nodes[NodeIndex].NodeType == NodeTypeLeaf)
  {
    int LeafNodeSamplesNum = (int)this->Nodes[NodeIndex].SamplesIndices.size();
...
void CKdTree::NearestSearchIteration(const int NodeIndex, const CVector & QueryPosition, int& NearestIndex, float& MinSquareDistance)
{
  if (this->Nodes[NodeIndex].NodeType == NodeTypeLeaf)
  {
		int LeafNodeSamplesNum = (int)this->Nodes[NodeIndex].SamplesIndices.size();
...
int CKdTree::NearestSearch(const CVector& QueryPosition, float SearchRadius)
{
...
void CKdTree::KNNSearch(const int RequiredKNNsNum, const CVector& QueryPosition, CKNNs* KNNs, float SearchRadius)
{
...
*/
void CKdTree::KNNSearchAAEllipse(const int RequiredKNNsNum, const CVector& QueryPosition, CKNNs* KNNs, const CVector & scale, float SearchRadius)
{
  KNNs->SamplesDistances.clear();
  KNNs->SamplesDistances.reserve(RequiredKNNsNum);
  KNNs->MaxSquaredDistance = SearchRadius;
...
*/
void CKdTree::KNNSearchEllipse(const int RequiredKNNsNum, const CVector& QueryPosition, CKNNs* KNNs, const CMatrix & M, const CMatrix & MInv, float SearchRadius)
{
  KNNs->SamplesDistances.clear();
  KNNs->SamplesDistances.reserve(RequiredKNNsNum);
  KNNs->MaxSquaredDistance = SearchRadius;
...
void CKdTree::ComputeNodeError(const int NodeIndex, bool Is2D)
{  
...
void CKdTree::ComputeSampleGradientKNNs(const int SampleIndex, const CKNNs &SKNNs)
{
